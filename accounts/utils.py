...
def get_proimage_name(title, image_name):
    ...
    img_path = os.path.join('profiles_images', img_name)
    return img_path


def get_procover_name(title, image_name):
    slug_title = slugify(title)
    imgName, imgExt = os.path.splitext(image_name)
    random_str = random_string_generator(size=6)
    img_name = slug_title + '-' + random_str + imgExt
    img_path = os.path.join('profiles_covers', img_name)
    return img_path
